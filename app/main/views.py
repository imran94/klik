...
from flask import render_template, session, redirect, url_for, \
        flash, abort, current_app, request, make_response
...
def index():
    ...
    pagination = query.order_by(Post.timestamp.desc()) \
        .paginate(
            page,
            per_page=current_app.config['KLIK_POSTS_PER_PAGE'],
            error_out=False)
...
def post(id):
    ...
    page = request.args.get('page', 1, type=int)
    per_page = current_app.config['KLIK_COMMENTS_PER_PAGE']
    if page == -1:
...
def followers(username):
    ...
    pagination = user.followers.paginate(
        page, per_page=current_app.config['KLIK_FOLLOWERS_PER_PAGE'],
        error_out=False)
